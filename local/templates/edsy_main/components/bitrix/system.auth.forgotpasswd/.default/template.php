...
if (!empty($arParams["~AUTH_RESULT"])) {
    $message = $arParams["~AUTH_RESULT"];
    if (is_array($message)) {
        if ($message['TYPE'] === 'OK') {
            $isSuccess = true;
            echo '<div class="edsys-success-message">Инструкции по восстановлению пароля были отправлены на указанный email.<br>При отсутствии сообщения, пожалуйста, проверьте папку "Спам" и убедитесь в правильности введенного email.</div>';
        } else {
            echo '<div class="edsys-error-message">' . $message['MESSAGE'] . '</div>';
        }
    } else {
        echo '<div class="edsys-error-message">' . $message . '</div>';
    }
}
?>
